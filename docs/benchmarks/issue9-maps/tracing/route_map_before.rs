...
fn attach(model:&TransitModel,p:Point<f64>)->Attach {
 let n=model.street_graph.rtree.nearest_neighbor(&p).unwrap().data;
 Attach {nodes:[n,n],costs:[0,0],edge:None,offset:0,point:model.street_graph.graph[n].geometry,segment:0,access:0}
}
fn edge_geometry(model:&TransitModel,e:EdgeIndex,from:NodeIndex)->Vec<[f64;2]> {
 let (a,b)=model.street_graph.graph.edge_endpoints(e).unwrap();
 vec![xy(model.street_graph.graph[from].geometry),xy(model.street_graph.graph[if a==from {b}else{a}].geometry)]
}
